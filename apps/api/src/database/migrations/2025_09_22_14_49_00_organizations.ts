...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await db.schema.alterTable('companies').renameTo('organizations').execute();
...
export async function down(db: Kysely<any>): Promise<void> {
  ...
  await db.schema.alterTable('organizations').dropColumn('parent_id').execute();
  ...
  await db.schema.alterTable('organizations').renameTo('companies').execute();
}
